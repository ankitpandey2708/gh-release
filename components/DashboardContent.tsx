...
const ReleaseChart = dynamic(
  ...
  {
    loading: () => <div className="h-64 md:h-96 bg-neutral-100 rounded-md animate-pulse" />,
    ssr: false,
  }
);
...
export function DashboardContent({ initialRepo }: DashboardContentProps = {}) {
  ...
  return (
    <main id="main" className="min-h-screen px-4 py-8 md:px-8 flex flex-col items-center bg-neutral-50 text-neutral-900 transition-colors duration-200">
      <ProgressBar loading={loading} />

      {/* Header with clear hierarchy */}
      <div className="flex flex-col sm:flex-row items-center gap-3 mb-8">
        <h1 className="text-h1 font-bold animate-fadeIn">GitHub releases dashboard</h1>
        {cached && (
      ...
      {data && !loading && data.length === 0 && (
        <div className="mt-8 w-full max-w-4xl p-8 bg-primary-light rounded-md text-center border border-primary/20">
          <h3 className="text-h3 font-medium text-neutral-900 mb-2">No releases found</h3>
      ...
      {filteredData && !loading && filteredData.length === 0 && data && data.length > 0 && (
        <div className="mt-8 w-full max-w-4xl p-8 bg-yellow-50 rounded-md text-center border border-yellow-200">
          <h3 className="text-h3 font-medium text-neutral-900 mb-2">No releases match filters</h3>
          <p className="text-body text-neutral-600">Try adjusting your filters above</p>
        </div>
      )}
