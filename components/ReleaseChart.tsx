...
export function ReleaseChart({ releases }: { releases: Release[] }) {
  ...
  if (releases.length === 0) {
    return (
      <div className="h-96 flex flex-col items-center justify-center bg-gray-50 rounded border-2 border-dashed border-gray-300">
  ...
  return (
    <div className={`w-full bg-white p-4 md:p-6 rounded border border-gray-300 transition-opacity duration-500 ${visible ? 'opacity-100' : 'opacity-0'}`}>
      <h2 className="text-lg md:text-xl font-bold mb-4 text-gray-900">Releases per Month</h2>
