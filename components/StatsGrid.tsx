...
export function StatsGrid({ releases }: { releases: Release[] }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <StatCard label="Avg Days" value={stats.avgDays} delay={100} visible={visible} tooltip="Average days between releases" />
      <StatCard label="Per Month" value={stats.perMonth} delay={200} visible={visible} tooltip="Average releases per month" />
      <StatCard label="Last Release" value={stats.lastRelease} delay={300} visible={visible} tooltip="Time since last release" />
    </div>
  );
}
